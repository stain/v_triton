...
#else
          // DOODLE
          /* Make sure we get at least two adjacent valid frames! */
          struct frame fr;
          h = decode_header(&fr, head);
          if (h)
          {
            unsigned int oldb[4]={0,0,0,0};
            int oldi,oldpos, extraread;
            struct buf *oldbuf;
            unsigned long head;

            /* Make sure we have enough data to skip */
            if (fr.framesize+4>mp->bsize)
              return -1;
            ...
            /* Skip the extra data */
            extraread = 0;
            while ((i<mp->bsize) && (extraread<fr.framesize+4))
            {
              /* get 4 bytes */
              b[0]=b[1]; b[1]=b[2]; b[2]=b[3];
              while(pos >= buf->size)
            ...
            /* Check, and return old 'i' value if it's ok! */
            h = head_check(head,fr.lay);
            if (h)
            {
              return oldi-3;
            ...
            {
              /* Restore old search state and continue searching from there otherwise! */
              buf = oldbuf;
              i = oldi;
              pos = oldpos;
